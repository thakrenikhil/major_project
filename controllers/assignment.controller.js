...
const createAssignment = async (req, res) => {
  try {
    ...
    if (!['faculty', 'admin'].includes(creator.role)) {
      return res.status(403).json({ error: 'Only faculty and admins can create assignments' });
    }

    // Validate required fields
    if (!course_id || !title || !description || !due_date || !max_marks || !weightage) {
    ...
    if (course.status !== 'IN_PROGRESS') {
      return res.status(400).json({ error: 'Course must be in progress to create assignments' });
    }
...
const publishAssignment = async (req, res) => {
  try {
    ...
    if (!['faculty', 'admin'].includes(publisher.role)) {
      return res.status(403).json({ error: 'Only faculty and admins can publish assignments' });
    }
    ...
    if (assignment.status !== 'draft') {
      return res.status(400).json({ error: 'Assignment is not in draft status' });
    }
...
const submitAssignment = async (req, res) => {
  try {
    ...
    const lateDays = isLate ? Math.ceil((submissionDate - dueDate) / (1000 * 60 * 60 * 24)) : 0;
...
const gradeAssignment = async (req, res) => {
  try {
    ...
    if (!['faculty', 'admin'].includes(grader.role)) {
      return res.status(403).json({ error: 'Only faculty and admins can grade assignments' });
    }
    ...
    if (submission.status !== 'submitted') {
      return res.status(400).json({ error: 'Submission is not in submitted status' });
    }
...
const getAssignments = async (req, res) => {
  try {
    ...
    if (user.role === 'student') {
      const enrollment = await Enrollment.findOne({ course_id, student_id: user._id });
      if (!enrollment) {
        return res.status(403).json({ error: 'Not enrolled in this course' });
      }
    } else if (user.role === 'nodal_officer' && !course.nodal_officer.equals(user._id)) {
...
const getSubmissions = async (req, res) => {
  try {
    ...
    // Check permissions
    if (user.role === 'student') {
      const enrollment = await Enrollment.findOne({ course_id: assignment.course_id, student_id: user._id });
      if (!enrollment) {
        return res.status(403).json({ error: 'Not enrolled in this course' });
      }
    }
